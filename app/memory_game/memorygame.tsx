        ...
        {imagesURL && imagesURL.length > 0 && 
            <div style={{ gridTemplateColumns: `repeat(${imagesURL.length}, minmax(0, 1fr))`}} className="grid gap-4 auto-cols-max p-20">
                {memoryCards.map((memoryCard)=> (
                    <div onClick={()=> !memoryCard.isFlipped ? handleSelection(memoryCard) : ''}>
                        <MemoryCard id={memoryCard.id} src={memoryCard.imageURL} isMatched={memoryCard.isMatched} isFlipped={memoryCard.isFlipped}/>
                    </div>    
                ))}
      </div>
    }   
